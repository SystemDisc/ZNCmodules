...
class CPerform : public CModule {
	...
	void List(const CString& sCommand) {
		...
		if (PutModule(Table) == 0) {
			PutModule("No commands in your disperform list.");
		}
	}
...
public:
	...
	virtual bool OnWebRequest(CWebSock& WebSock, const CString& sPageName, CTemplate& Tmpl) {
		if (sPageName != "index") {
			// only accept requests to /mods/disperform/
			return false;
		}

		if (WebSock.IsPost()) {
			VCString vsPerf;
			WebSock.GetRawParam("disperform", true).Split("\n", vsPerf, false);
			m_vPerform.clear();
...
template<> void TModInfo<CPerform>(CModInfo& Info) {
	Info.SetWikiPage("disperform");
}

MODULEDEFS(CPerform, "Keeps a list of commands to be executed when client disconnects from ZNC.")
